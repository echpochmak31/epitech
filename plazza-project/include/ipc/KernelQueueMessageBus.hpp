...
typedef std::function<void(std::shared_ptr<IpcMessage> &message)>
    IpcCallbackFunction;
...
class KernelQueueMessageBus : public IMessageBus {
 protected:
  ...
  std::shared_ptr<std::unordered_map<IpcAddress, IpcCallbackFunction>>
      directCallbacks;
 ...
 public:
  explicit KernelQueueMessageBus(std::shared_ptr<Logger> logger)
      : logger(logger) {
    ...
    directCallbacks =
        std::make_shared<std::unordered_map<IpcAddress, IpcCallbackFunction>>();
    callbackGroups =
        std::make_shared<std::unordered_map<IpcAddress, CallbackGroup>>();
  }

  ~KernelQueueMessageBus() { msgctl(msgid, IPC_RMID, nullptr); }
  ...
  void runMessageHandling() override {
    while (!stopFlag) {
      ...
      std::shared_ptr<IpcMessage> message =
          IpcMessage::deserialize(serializedMessage);
...

      {
        std::lock_guard<std::mutex> lock(
            callbackMtx);  // Lock for access to directCallbacks and
                           // callbackGroups

        if (directCallbacks->find(receiverIpcAddress) !=
            directCallbacks->end()) {
          logger->logDebug("P2P callback has been found for message: " +
                           serializedMessage);
        ...
        if (callbackGroups->find(receiverIpcAddress) != callbackGroups->end()) {
          CallbackGroup targetCallbackGroup =
              (*callbackGroups)[receiverIpcAddress];
          if (targetCallbackGroup.find(routingKey) !=
              targetCallbackGroup.end()) {
            logger->logDebug(
                "Callback with routing key has been found for message: " +
                serializedMessage);
      ...
      logger->logError("No suitable callback was found for message: " +
                       serializedMessage);
    }
  }
  ...
  void subscribe(const std::string &receiverIpcAddress,
                 ...
                 std::function<void(std::shared_ptr<IpcMessage> &message)>
                     callback) override {
    std::lock_guard<std::mutex> lock(
        callbackMtx);  // Lock for modifying callbackGroups
  ...
  void subscribe(const std::string &receiverIpcAddress,
                 std::function<void(std::shared_ptr<IpcMessage> &message)>
                     callback) override {
    std::lock_guard<std::mutex> lock(
        callbackMtx);  // Lock for modifying directCallbacks
 ...
 private:
  ...
  std::atomic<bool> stopFlag{
      false};  // Atomic flag to stop the message handling loop
};
