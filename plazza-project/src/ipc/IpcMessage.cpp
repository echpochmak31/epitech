...
IpcMessage::IpcMessage(const std::string &senderIpcAddress,
                       const std::string &receiverIpcAddress,
                       const std::string &routingKey,
                       const std::string &serializedPayload)
    : senderIpcAddress(senderIpcAddress),
      receiverIpcAddress(receiverIpcAddress),
...
std::string IpcMessage::serialize() const {
  ...
  oss << senderIpcAddress << ":" << receiverIpcAddress << ":" << routingKey
      << ":" << dispatchTimeSinceEpoch << ":" << serializedPayload;
  return oss.str();
}
...
std::shared_ptr<IpcMessage> IpcMessage::deserialize(const std::string &data) {
  ...
  auto result = std::make_shared<IpcMessage>(sender, receiver, routingKey,
                                             serializedPayload);
...
std::string IpcMessage::getSender() const { return senderIpcAddress; }

std::string IpcMessage::getReceiver() const { return receiverIpcAddress; }

std::string IpcMessage::getRoutingKey() const { return routingKey; }
