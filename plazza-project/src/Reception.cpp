...
Reception::Reception(
    ...
    KitchenParams kitchenParams, std::shared_ptr<IMessageBus> messageBus,
...
Reception::~Reception() { receptionEnabled = false; }
...
std::vector<std::pair<IpcAddress, KitchenStatusDto>> Reception::pollKitchens() {
  ...
  auto onStatusReceived = [this,
                           &responses](std::shared_ptr<IpcMessage> &message) {
    if (message->getRoutingKey() != IpcRoutingKeyHolder::GetKitchenStatus) {
      ...
      errorMessage << "Unexpected message received. Expected routing key is: "
                   ...
                   << " Actual is: " << message->getRoutingKey();
  ...
  messageBus->subscribe(receptionIpcAddress,
                        IpcRoutingKeyHolder::GetKitchenStatus,
                        onStatusReceived);

  for (const auto &[Pid, IpcAddress] : kitchenPool) {
    auto getStatusRequest = std::make_shared<IpcMessage>(
        receptionIpcAddress, IpcAddress, IpcRoutingKeyHolder::GetKitchenStatus);
  ...
  if (responses.size() != fixedKitchenNumber) {
    ...
    wargingMessage << "Some kitchens failed to respond. " << responses.size()
                   << " out of " << static_cast<int>(fixedKitchenNumber)
                   << " expected responses has been received.";
...
void Reception::assignPizza(IpcAddress kitchenIpcAddress,
                            OrderedPizzaDto orderedPizzaDto) {
  auto message = std::make_shared<IpcMessage>(
      receptionIpcAddress, kitchenIpcAddress,
      IpcRoutingKeyHolder::AcceptOrderedPizza, orderedPizzaDto.serialize());

  messageBus->publish(message);
}

void Reception::distributeOrderedPizzas(
    std::vector<std::pair<IpcAddress, int>> targetKitchens) {
  ...
  while (!queuedPizzas->empty() && commonCapacity > 0) {
    ...
    for (auto [kitchenIpcAddress, kitchenCapacity] : targetKitchens) {
      if (currentCapacities[kitchenIpcAddress] == 0) continue;
...
KitchenDetails Reception::createNewKitchen() {
  ...
  if (pid == 0) {
    ...
    auto kitchen = std::make_shared<Kitchen>(logger, kitchenIpcAddress,
                                             messageBus, kitchenParams);
  ...
  auto onCloseKitchen = [this, kitchenIpcAddress](
  ...
  auto onOrderedPizzaReady = [this, kitchenIpcAddress](
                                 std::shared_ptr<IpcMessage> &message) {
  ...
  messageBus->subscribe(receptionIpcAddress, IpcRoutingKeyHolder::CloseKitchen,
                        onCloseKitchen);
  messageBus->subscribe(receptionIpcAddress,
  ...
  messageBus->subscribe(
      kitchenIpcAddress,
      std::bind(&Kitchen::handleMessage,
                std::make_shared<Kitchen>(logger, kitchenIpcAddress, messageBus,
                                          kitchenParams),
                std::placeholders::_1));
...
void Reception::closeKitchen(IpcAddress kitchenIpcAddress) {
  ...
  if (it != kitchenPool.end()) {
    ...
    if (kill(pid, SIGTERM) == -1) {
      auto errorMessage = "Failed to terminate kitchen process with PID: " +
                          std::to_string(pid);
      logger->logError(errorMessage);
      throw std::runtime_error(errorMessage);
    }
    ...
    logger->logDebug("Kitchen closed with PID: " + std::to_string(pid) +
                     ", IPC Address: " + kitchenIpcAddress);
  } else {
    auto errorMessage =
        "Kitchen with IPC Address: " + kitchenIpcAddress + " not found";
    logger->logError(errorMessage);
    throw std::runtime_error(errorMessage);
  }
}
...
void Reception::stop() { receptionEnabled = false; }
...
void Reception::assignPizzas() {
  std::vector<std::pair<IpcAddress, int>>
      targetKitchens;  // address - number of pizzas ready to accept
  ...
  for (const auto &[ipcAddress, status] : statuses) {
    int count = status.totalCookNumber * MAX_ORDERED_PIZZAS_MULTIPLIER -
                status.queuedPizzaNumber;
    if (count > 0) {
  ...
  if (deficiency > 0) {
    int newKitchensCount =
        std::ceil(static_cast<double>(deficiency) /
    ...
    for (int i = 0; i < newKitchensCount; i++) {
      ...
      targetKitchens.push_back(std::make_pair(
          kitchenDetails.IpcAddress,
          kitchenParams.cooksNumber * MAX_ORDERED_PIZZAS_MULTIPLIER));
    }
  }
...
std::vector<std::pair<IpcAddress, KitchenStatusDto>>
Reception::closeIdleKitchens(
  ...
  for (const auto &[ipcAddress, status] : statuses) {
    ...
    auto lastActiveTime =
        std::chrono::system_clock::from_time_t(status.lastActiveTimeSinceEpoch);
    auto idleDuration =
        std::chrono::duration_cast<std::chrono::seconds>(lastActiveTime - now)
            .count();

    if (idleDuration > KITCHEN_IDLE_TIME_LIMIT_IN_SECONDS &&
        status.queuedPizzaNumber == 0) {
...
std::string Reception::getStatus() {
  ...
  for (const auto &[ipcAddress, status] : nonIdleStatuses) {
    std::chrono::system_clock::duration duration(
        status.lastActiveTimeSinceEpoch);
    std::chrono::system_clock::time_point lastActiveTime(duration);

    oss << "Status of kitchen: " << ipcAddress << ";\t";
    oss << "Available cooks: " << status.availableCookNumber << " of "
        << status.totalCookNumber << ";\t";
